<template>
  <div class="route-admin">
    ...
    <el-table :data="tableVisibleData" border style="width: 100%" ref="multipleTable" @selection-change="handleSelectionChange">
      <el-table-column prop="pk_ROUTE" label="Route ID" width="245"></el-table-column>
    ...
    <el-dialog title="详细信息" :visible.sync="detailVisible" :modal="false">
      <table border="0" class="add-message" cellspacing="3" cellpadding="3">
        <tr>
          <th> key </th>
          <th> value </th>
        </tr>
      ...
      <!-- <el-table :data="detail" stripe style="width: 100%">
        <el-table-column v-for="(value, key) in detail" prop="key" label="日期" width="180">
        </el-table-column>
      </el-table> -->
      <span slot="footer">
...
export default {
  data() {
    return {
      api: this.$store.state.api + '/api/route-config',
      tableData: [],
      tableVisibleData: [],
      cur_page: 1,
      select_word: '',
  ...
  methods: {
    ...
    add() {
      ...
      var loading = this.$loading({
        ...
        //   loading.close();
        // }, 2000);
        this.axios.get(this.$store.state.api + '/sync-route').then((response) => {
          loading.close()
          this.$notify({
            title: '同步成功',
